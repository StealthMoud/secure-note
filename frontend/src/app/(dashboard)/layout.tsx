...
function DashboardShell({ children, isSidebarOpen, setIsSidebarOpen }: {
    ...
    useEffect(() => {
        ...
        const tabFromPath = pathname === '/' || pathname === '/dashboard' ? 'dashboard' : pathname.replace('/', '');
        if (tabFromPath !== activeTab) {
            setActiveTab(tabFromPath);
        }

        const pageTitle = titleMap[tabFromPath] || 'Dashboard';
        document.title = `${pageTitle} | Secure Note`;
    }, [pathname, setActiveTab, activeTab, user, router]);
